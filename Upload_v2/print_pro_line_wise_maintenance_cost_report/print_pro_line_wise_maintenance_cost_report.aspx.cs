...
public partial class print_pro_line_wise_maintenance_cost_report : COMM_SessionCheck
{
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
        ...
        rpt_file = db_conf_report_configuration_mst.Get_rpt_file_name("print_pro_line_wise_maintenance_cost_report.aspx", 1, Handle.ToInt32(Session["comp_id"]));
        if (String.IsNullOrEmpty(rpt_file))
        {
            rpt_file = "print_pro_line_wise_maintenance_cost_report.rpt";
        }
    ...
    public void bind_data()
    {
        ...
        int line = 0;
            try { line = db_ctlg_item_mst.GetIdByCode(txt_line.Text, Handle.ToInt32(Session["comp_id"])); }
            catch { line = 0; }
                
        int machine = 0;
            try { machine = db_ctlg_item_mst.GetIdByCode(txt_machine.Text, Handle.ToInt32(Session["comp_id"])); }
            catch { machine = 0; }
        ...
        CrystalReportSource1.ReportDocument.SetParameterValue("com_id", Handle.ToInt32(Session["comp_id"]));
          ...
          CrystalReportSource1.ReportDocument.SetParameterValue("line", line);
          CrystalReportSource1.ReportDocument.SetParameterValue("machine", machine);
