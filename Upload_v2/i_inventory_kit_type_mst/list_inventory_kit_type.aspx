<%@ Page Language="C#" MasterPageFile="~/Include/Master/list.master" AutoEventWireup="true"
    CodeFile="list_inventory_kit_type.aspx.cs" Inherits="list_inventory_kit_type" %>
    ...
    <!-- widget grid -->
        ...
        function SaveDef()
        {
            ...
            if(flg==0)
            {
                ...
                var DatStr={'uwgs_set_name':$("#txtDefName").val(),'uwgs_grid_name':'Manage_Kit_Type_Master','uwgs_grid_state':JSON.stringify(stat)}
        ...
        var viewLog = function (event) {
            ...
            if(dataRecord.id !="")
            {
                $.ajax({
                    type: "POST",
                    url: "ajax_inventory.aspx",
                    async: true,
                    data: "slv_ktm_id="+dataRecord.id+"&sign=Manage_Kit_Type_Master",
                    success: function (data) {
                        if (data.toString() != '') 
                        {                                    
        ...
        var Editclick = function (event)
        {
            var id = event.target.id;
            var dataRecord = $("#jqxgrid").jqxGrid('getrowdata', id);
            window.location.href = "form_inventory_kit_type.aspx?id=" + dataRecord.id;
        }
        var Deleteclick = function (event) {
            ...
            ConfirmBox("Are You Sure?","Do you want to delete this record?").done(function(){
                $.ajax({
                    ...
                    url: "ajax_inventory.aspx",
                    async: true,
                    data: "del_ktm_id=" + dataRecord.id+"&sign=Manage_Kit_Type_Master",
                    success: function (data) {
                        if (data.toString() == '1') {
                            alert('Manage_Kit_Type_Master deleted successfully.');
...
{ name: 'ktm_kit_type_name', type: 'string', colName: '' },
...
{ name: 'ktm_kit_type_name', type: 'string', colName: '' }],
        ...
        function GetDataAJAX()
        {
            $.ajax({
                ...
                url: "ajax_inventory.aspx",
                async: true,
                data: "gktm_user_id=<%=Session["user_id"]%>&sign=Manage_Kit_Type_Master",
                success: function (data) {
                    if (data.toString() != '') 
                    {                                    
                        datafields = data;                                    
        ...
        function GetDefData()
        {
            ...
            $.ajax({
                ...
                url: "<%=Session["base_url"]%>ajax_comm.aspx",
                async: false,
                data: "GridDef_user_id=<%=Session["user_id"]%>&Grid_name=Manage_Kit_Type_Master",
                success: function (data) {
                    if (data.toString() != '') 
                    {                                    
            ...
            $("#jqxgrid").jqxGrid(
            {
                ...
                rendertoolbar: function (toolbar) {
                    ...
                    addButton.click(function (event) {
                        window.location.href = "form_inventory_kit_type.aspx";
                    });
                    // Export In Excel
                    ExportBtn.click(function (event) 
                    {
                        $("#jqxgrid").jqxGrid('exportdata', 'xls', 'Manage_Kit_Type_Master_Detail');
                    });
                    //Deleted Log
                    deletedLogButton.click(function (event) {
                        ...
                        $.ajax({
                            ...
                            url: "ajax_inventory.aspx",
                            async: true,
                            data: "dlv_ktm_user_id=<%=Session["user_id"]%>&sign=Manage_Kit_Type_Master",
                            success: function (data) {
                                if (data.toString() != '') 
                                {                                    
                                    datafieldsViewLog = data;                                    
...
{ text: '', datafield: 'ktm_kit_type_name',filtertype: 'input' },
