<%@ Page Language="C#" MasterPageFile="~/Include/Master/list.master" AutoEventWireup="true"
    CodeFile="list_sales_inventory_kit.aspx.cs" Inherits="list_sales_inventory_kit" %>
    ...
    <!-- widget grid -->
        ...
        function SaveDef()
        {
            ...
            if(flg==0)
            {
                ...
                var DatStr={'uwgs_set_name':$("#txtDefName").val(),'uwgs_grid_name':'Kit_Master','uwgs_grid_state':JSON.stringify(stat)}
        ...
        var viewLog = function (event) {
            ...
            if(dataRecord.id !="")
            {
                $.ajax({
                    ...
                    url: "ajax_sales.aspx",
                    async: true,
                    data: "slv_ikm_id="+dataRecord.id+"&sign=Kit_Master",
                    success: function (data) {
                        if (data.toString() != '') 
                        {                                    
        ...
        var Editclick = function (event)
        {
            var id = event.target.id;
            var dataRecord = $("#jqxgrid").jqxGrid('getrowdata', id);
            window.location.href = "form_sales_inventory_kit.aspx?id=" + dataRecord.id;
        }
        var Deleteclick = function (event) {
            ...
            ConfirmBox("Are You Sure?","Do you want to delete this record?").done(function(){
                $.ajax({
                    ...
                    url: "ajax_sales.aspx",
                    async: true,
                    data: "del_ikm_id=" + dataRecord.id+"&sign=Kit_Master",
                    success: function (data) {
                        if (data.toString() == '1') {
                            alert('Kit_Master deleted successfully.');
...
{ name: 'ikm_name', type: '', colName: '' },
{ name: 'ikm_kit_type_id', type: '', colName: '' },
...
{ name: 'ikm_kit_type_id', type: '', colName: '' }],
        ...
        function GetDataAJAX()
        {
            $.ajax({
                ...
                url: "ajax_sales.aspx",
                async: true,
                data: "gikm_user_id=<%=Session["user_id"]%>&sign=Kit_Master",
                success: function (data) {
                    if (data.toString() != '') 
                    {                                    
                        datafields = data;                                    
        ...
        function GetDefData()
        {
            ...
            $.ajax({
                ...
                url: "<%=Session["base_url"]%>ajax_comm.aspx",
                async: false,
                data: "GridDef_user_id=<%=Session["user_id"]%>&Grid_name=Kit_Master",
                success: function (data) {
                    if (data.toString() != '') 
                    {                                    
            ...
            $("#jqxgrid").jqxGrid(
            {
                ...
                rendertoolbar: function (toolbar) {
                    ...
                    addButton.click(function (event) {
                        window.location.href = "form_sales_inventory_kit.aspx";
                    });
                    // Export In Excel
                    ExportBtn.click(function (event) 
                    {
                        $("#jqxgrid").jqxGrid('exportdata', 'xls', 'Kit_Master_Detail');
                    });
                    //Deleted Log
                    deletedLogButton.click(function (event) {
                        ...
                        $.ajax({
                            ...
                            url: "ajax_sales.aspx",
                            async: true,
                            data: "dlv_ikm_user_id=<%=Session["user_id"]%>&sign=Kit_Master",
                            success: function (data) {
                                if (data.toString() != '') 
                                {                                    
                                    datafieldsViewLog = data;                                    
...
{ text: '', datafield: 'ikm_name',filtertype: '' },
{ text: '', datafield: 'ikm_kit_type_id',filtertype: '' },
