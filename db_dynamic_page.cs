...
/// </summary>
public class db_dynamic_page
{
    public db_dynamic_page()
    {
        //
    ...
    #region GetSPALTER_QUERY Function
    public static String GetSPALTER_QUERY(String conString, String obj_name)
    {
...
set @sql='ALTER PROCEDURE [dbo].[' + @obj_name+']'

Declare @Object_id bigint=object_id(@obj_name)
...
BEGIN
    select  
        @sql + CHAR(10) + string_agg(name + ' ' + type_name(user_type_id),',' + CHAR(10))
        from sys.parameters where object_id = object_id(@obj_name)
END
ELSE
BEGIN
    select  'Invalid'
...
set @triggerName =replace(@tbl_name,'i_','trg_')
...
BEGIN
	...
	SELECT @event_type = 0 
	if(@event_type in (2))
	insert into '+@synForlogtbl+' select case when modify_dnt IS NULL then 1 else @event_type end,GETDATE(),* from deleted
    ...
    public static String CreateInsertProcedure(string conString, string tblName, string objName, string uniqueCheck, string uniqueCheckLabels, string InsertOnly, string autoGenerated)
    {
        ...
        comm.CommandText = @"if(@obj_name='')
                            ...
                            and [name] not like '%modify_by%' and [name] not like '%modify_ip%' and [name] not like '%modify_dnt%'
                             ...
                             if(@unique_check !='')
                             set @cond_unique_1='IF(not exists(select * from '+@tbl_name+' with(nolock) where '+@unique+' ))
    BEGIN'
...
";
    ...
    /// <param name="objName"></param>
    /// <returns></returns>
    public static String CreateDeleteProcedure(string conString, string tblName, string objName)
    {
        // get a configured DbCommand object
        DAL_GenericDataAccess gda = new DAL_GenericDataAccess();
        DbCommand comm = gda.CreateCommand(conString);
...
BEGIN
  ...
  update '+@tbl_name+' set modify_by=@user_id,modify_dnt=GETDATE(),modify_ip=@ip Where '+@primary_field+' = @'+@primary_field+'  and comp_id=@comp_id
  Delete from '+@tbl_name+' Where '+@primary_field+' = @'+@primary_field+'  and comp_id=@comp_id
...
";
    ...
    /// <param name="objName"></param>
    /// <returns></returns>
    public static String CreateSelectByIDProcedure(string conString, string tableName, string objName)
    {
        // get a configured DbCommand object
        DAL_GenericDataAccess gda = new DAL_GenericDataAccess();
        DbCommand comm = gda.CreateCommand(conString);
...
BEGIN
	SELECT 
	'+@clmn+'
	from '+@tbl_name+' as '+@prefx+' with(nolock) where '+@prefx+'.'+@primary_field+'=@'+@primary_field+'		 and '+@prefx+'.comp_id=@comp_id
END
'
...
SELECT euser.usrm_name as create_by_user,
...
";
    ...
    /// <returns></returns>
    public static String CreateLogSelectProcedure(string conString, string tbl_name, string obj_name)
    {
        // get a configured DbCommand object
        DAL_GenericDataAccess gda = new DAL_GenericDataAccess();
        DbCommand comm = gda.CreateCommand(conString);
...
left join i_conf_user_mst euser with(nolock) on '+@prefx+'.create_by = euser.id
left join i_conf_user_mst muser with(nolock) on '+@prefx+'.modify_by = muser.id
where  '+@prefx+'.id=@id and '+@prefx+'.comp_id =@comp_id
...
";
    ...
    /// <param name="objName"></param>
    /// <returns></returns>
    public static String CreateDeleteProcedure_Detail(string conString, string tblName, string objName, string primary_field)
    {
        // get a configured DbCommand object
        DAL_GenericDataAccess gda = new DAL_GenericDataAccess();
        DbCommand comm = gda.CreateCommand(conString);
...
";
        ...
        param = comm.CreateParameter();
        param.ParameterName = "@primary_field";
        param.Value = primary_field;
        param.DbType = DbType.String;
        comm.Parameters.Add(param);
    ...
    /// <param name="objName"></param>
    /// <returns></returns>
    public static String CreateSelectByIDProcedure_Detail(string conString, string tableName,string objName, string primary_field)
    {
        // get a configured DbCommand object
        DAL_GenericDataAccess gda = new DAL_GenericDataAccess();
        DbCommand comm = gda.CreateCommand(conString);
