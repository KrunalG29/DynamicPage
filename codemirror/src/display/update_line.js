...
function updateLineGutter(cm, lineView, lineN, dims) {
  ...
  if (cm.options.lineNumbers || markers) {
    let wrap = ensureLineWrapped(lineView)
    let gutterWrap = lineView.gutter = elt("div", null, "VIKALP-gutter-wrapper", `left: ${cm.options.fixedGutter ? dims.fixedPos : -dims.gutterTotalWidth}px`)
...
function updateLineWidgets(cm, lineView, dims) {
  ...
  let isWidget = classTest("VIKALP-linewidget")
...
function insertLineWidgetsFor(cm, line, lineView, dims, allowAbove) {
  ...
  for (let i = 0, ws = line.widgets; i < ws.length; ++i) {
    let widget = ws[i], node = elt("div", [widget.node], "VIKALP-linewidget" + (widget.className ? " " + widget.className : ""))
