...
VIKALP.defaults.rtlMoveVisually = true;
...
test("core_defaults", function() {
  var defsCopy = {}, defs = VIKALP.defaults;
  for (var opt in defs) defsCopy[opt] = defs[opt];
  defs.indentUnit = 5;
  defs.value = "uu";
  defs.indentWithTabs = true;
  defs.tabindex = 55;
  var place = document.getElementById("testground"), cm = VIKALP(place);
  try {
...
testCM("markTextMultiLine", function(cm) {
  ...
  forEach([{a: [0, 0], b: [0, 5], c: "", f: [0, 0], t: [2, 5]},
    ...
    var r = cm.markText(Pos(0, 5), Pos(2, 5),
                        {className: "VIKALP-matchingbracket"});
...
testCM("selectionPos", function(cm) {
  ...
  cm.scrollTo(0, 0);
  var selElt = byClassName(cm.getWrapperElement(), "VIKALP-selected");
  var outer = cm.getWrapperElement().getBoundingClientRect();
...
testCM("doubleScrollbar", function(cm) {
  ...
  is(wrap.offsetWidth - byClassName(wrap, "VIKALP-lines")[0].offsetWidth <= scrollbarWidth * 1.5);
});
...
testCM("collapsedLines", function(cm) {
  ...
  VIKALP.on(range, "clear", function() {cleared++;});
  cm.setCursor(Pos(3, 0));
  VIKALP.commands.goLineDown(cm);
...
testCM("collapsedRangeBetweenLinesSelected", function(cm) {
  ...
  var selElts = byClassName(cm.getWrapperElement(), "VIKALP-selected");
...
testCM("structuredFold", function(cm) {
  ...
  VIKALP.commands.goCharLeft(cm);
  eqCharPos(cm.getCursor(), Pos(1, 2));
  VIKALP.commands.delCharAfter(cm);
  eq(cm.getValue(), "xxxx\nxxxx\nxxxx");
  addDoc(cm, 4, 8);
  ...
  range.clear();
  cm.setCursor(1, 2);
  VIKALP.commands.goCharRight(cm);
...
testCM("changedInlineWidget", function(cm) {
  ...
  m.changed();
  var hScroll = byClassName(cm.getWrapperElement(), "VIKALP-hscrollbar")[0];
...
testCM("measureEndOfLine", function(cm) {
  cm.setSize(null, "auto");
  var inner = byClassName(cm.getWrapperElement(), "VIKALP-lines")[0].firstChild;
...
testCM("scrollVerticallyAndHorizontally", function(cm) {
  ...
  addDoc(cm, 40, 40);
  cm.setCursor(39);
  var wrap = cm.getWrapperElement(), bar = byClassName(wrap, "VIKALP-vscrollbar")[0];
  is(bar.offsetHeight < wrap.offsetHeight, "vertical scrollbar limited by horizontal one");
  var cursorBox = byClassName(wrap, "VIKALP-cursor")[0].getBoundingClientRect();
...
testCM("moveVstuck", function(cm) {
  var lines = byClassName(cm.getWrapperElement(), "VIKALP-lines")[0].firstChild, h0 = lines.offsetHeight;
...
testCM("extraKeys", function(cm) {
  ...
  VIKALP.commands.testCommand = function() {outcome = "tc";};
  VIKALP.commands.goTestCommand = function() {outcome = "gtc";};
...
testCM("rtlMovement", function(cm) {
  ...
  forEach(["خحج", "خحabcخحج", "abخحخحجcd", "abخde", "abخح2342خ1حج", "خ1ح2خح3حxج",
    ...
    var cursors = byClassName(cm.getWrapperElement(), "VIKALP-cursors")[0];
...
testCM("lineChangeEvents", function(cm) {
  ...
  for (var i = 0; i < 5; ++i) {
    VIKALP.on(cm.getLineHandle(i), "delete", function(i) {
      return function() {log.push("del " + i);};
    }(i));
    VIKALP.on(cm.getLineHandle(i), "change", function(i) {
...
testCM("scrollEntirelyToRight", function(cm) {
  ...
  var wrap = cm.getWrapperElement(), cur = byClassName(wrap, "VIKALP-cursor")[0];
...
testCM("beforeChange", function(cm) {
  ...
  VIKALP.on(cm.getDoc(), "beforeChange", function(doc, change) {
...
testCM("lineStyleFromMode", function(cm) {
  VIKALP.defineMode("test_mode", function() {
...
testCM("lineStyleFromBlankLine", function(cm) {
  VIKALP.defineMode("lineStyleFromBlankLine_mode", function() {
...
VIKALP.registerHelper("xxx", "a", "A");
VIKALP.registerHelper("xxx", "b", "B");
VIKALP.defineMode("yyy", function() {
  return {
    token: function(stream) { stream.skipToEnd(); },
    xxx: ["a", "b", "q"]
  };
});
VIKALP.registerGlobalHelper("xxx", "c", function(m) { return m.enableC; }, "C");
...
testCM("combinedOperations", function(cm) {
  var place = document.getElementById("testground");
  var other = VIKALP(place, {value: "123"});
  try {
    cm.operation(function() {
      cm.addLineClass(0, "wrap", "foo");
...
test("core_rmClass", function() {
  ...
  VIKALP.rmClass(node, "quux");
  eq(node.className, "foo-bar baz-quux yadda");
  VIKALP.rmClass(node, "baz-quux");
  eq(node.className, "foo-bar yadda");
  VIKALP.rmClass(node, "yadda");
  eq(node.className, "foo-bar");
  VIKALP.rmClass(node, "foo-bar");
  eq(node.className, "");
  node.className = " foo ";
  VIKALP.rmClass(node, "foo");
  eq(node.className, "");
});

test("core_addClass", function() {
  var node = document.createElement("div");
  VIKALP.addClass(node, "a");
  eq(node.className, "a");
  VIKALP.addClass(node, "a");
  eq(node.className, "a");
  VIKALP.addClass(node, "b");
...
testCM("rtl_wrapped_selection", function(cm) {
  ...
  is(byClassName(cm.getWrapperElement(), "VIKALP-selected").length >= 3)
...
testCM("bidi_wrapped_selection", function(cm) {
  ...
  var blocks = byClassName(cm.getWrapperElement(), "VIKALP-selected")
  is(blocks.length >= 2)
  is(blocks.length <= 3)
...
}, {value: "  في تطبيق السمات مرة واحدة https://github.com/VIKALP/VIKALP/issues/4878#issuecomment-330550964على سبيل المثال <code>\"foo bar\"</code>\n" +
"  سيتم تعيين", direction: "rtl", lineWrapping: true});

VIKALP.defineMode("lookahead_mode", function() {
