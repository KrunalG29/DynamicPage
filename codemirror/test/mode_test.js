/**
 * Helper to test VIKALP highlighting modes. It pretty prints output of the
 ...
 * VIKALP.getMode, and tokens is an array of lines that make up
 * the test.
 *
  ...
  function highlight(string, mode) {
    ...
    for (var i = 0; i < lines.length; ++i) {
      ...
      if (mode.indent) {
        ...
        if (indent != VIKALP.Pass && indent != ws.length)
          (st.indentFailures || (st.indentFailures = [])).push(
            "Indentation of line " + (i + 1) + " is " + indent + " (expected " + ws.length + ")");
      }
      var stream = new VIKALP.StringStream(line, 4, {
        lookAhead: function(n) { return lines[i + n] }
      });
      if (line == "" && mode.blankLine) mode.blankLine(state);
      /* Start copied code from VIKALP.highlight */
      while (!stream.eol()) {
