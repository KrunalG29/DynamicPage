(function() {
  // A minilanguage for instantiating linked VIKALP instances and Docs
  function instantiateSpec(spec, place, opts) {
    ...
    while (spec) {
      ...
      if (m[3]) {
        cur = isDoc ? VIKALP.Doc(m[3]) : VIKALP(place, clone(opts, {value: m[3]}));
      } else {
        var other = m[5];
        if (!names.hasOwnProperty(other)) {
          names[other] = editors.length;
          editors.push(VIKALP(place, opts));
        }
        ...
        cur = isDoc ? doc : VIKALP(place, clone(opts, {value: doc}));
      }
      ...
      try {
        ...
        if (!successful || verbose) {
          ...
          for (var i = 0; i < editors.length; ++i)
            if (editors[i] instanceof VIKALP)
  ...
  testDoc("bareDoc", "A*='foo' B*<A C<B", function(a, b, c) {
    is(a instanceof VIKALP.Doc);
    is(b instanceof VIKALP.Doc);
    is(c instanceof VIKALP);
    eqAll("foo", a, b, c);
  ...
  testDoc("docKeepsMode", "A='1+1'", function(a) {
    var other = VIKALP.Doc("hi", "text/x-markdown");
  ...
  testDoc("sharedMarker", "A='ab\ncd\nef\ngh' B<A C<~A/1-2", function(a, b, c) {
    ...
    var cleared = 0;
    VIKALP.on(mark, "clear", function() {++cleared;});
