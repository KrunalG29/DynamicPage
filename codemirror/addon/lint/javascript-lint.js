...
(function(mod) {
  ...
  function validator(text, options) {
    if (!window.JSHINT) {
      if (window.console) {
        window.console.error("Error: window.JSHINT not defined, VIKALP JavaScript linting cannot run.");
      }
      return [];
    }
  ...
  VIKALP.registerHelper("lint", "javascript", validator);
    ...
    for ( var i = 0; i < errors.length; i++) {
      ...
      if (error) {
        ...
        var hint = {
          ...
          from: VIKALP.Pos(error.line - 1, start),
          to: VIKALP.Pos(error.line - 1, end)
        };
