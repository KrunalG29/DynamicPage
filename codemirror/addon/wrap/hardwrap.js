...
(function(mod) {
  ...
  function wrapRange(cm, from, to, options) {
    ...
    if (changes.length) cm.operation(function() {
      for (var i = 0; i < changes.length; ++i) {
        ...
        if (change.text || VIKALP.cmpPos(change.from, change.to))
          cm.replaceRange(change.text, change.from, change.to);
      }
    });
    return changes.length ? {from: changes[0].from, to: VIKALP.changeEnd(changes[changes.length - 1])} : null;
  }

  VIKALP.defineExtension("wrapParagraph", function(pos, options) {
    options = options || {};
  ...
  VIKALP.commands.wrapLines = function(cm) {
  ...
  VIKALP.defineExtension("wrapRange", function(from, to, options) {
    return wrapRange(this, from, to, options || {});
  });

  VIKALP.defineExtension("wrapParagraphsInRange", function(from, to, options) {
    options = options || {};
    var cm = this, paras = [];
