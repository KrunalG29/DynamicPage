...
(function(mod) {
  ...
  function continueComment(cm) {
    ...
    for (var i = 0; i < ranges.length; i++) {
      ...
      if (!/\bcomment\b/.test(cm.getTokenTypeAt(pos))) return VIKALP.Pass;
      var modeHere = cm.getModeAt(pos)
      if (!mode) mode = modeHere;
      else if (mode != modeHere) return VIKALP.Pass;
      ...
      if (blockStart && mode.blockCommentContinue) {
        ...
        if (end != -1 && end == pos.ch - mode.blockCommentEnd.length ||
          ...
          if (nonspaceAfter(0, line) >= found) {
            ...
            found = VIKALP.countColumn(line, found, tabSize);
      ...
      if (insert == null) return VIKALP.Pass;
      inserts[i] = "\n" + insert;
    }
  ...
  VIKALP.defineOption("continueComments", null, function(cm, val, prev) {
    if (prev && prev != VIKALP.Init)
